...
public class Collage {
    ...
    public Collage (String filename) {
        ...
        originalPicture = new Picture(filename); //CHANGe
    ...
    public void makeCollage () {
        Picture collageN = new Picture(tileDimension, tileDimension);
        scale(collagePicture, collageN);
        for(int row = 0;row<tileDimension*collageDimension;row++){
            for(int column = 0;column<tileDimension*collageDimension;column++){
                collagePicture.set(row,column,collageN.get(row%tileDimension,column%tileDimension));
            }
        }
    ...
    public void colorizeTile (String component,  int collageCol, int collageRow) {
        int colCol = collageCol*tileDimension;
        int colRow = collageRow*tileDimension;
        if(component.equals("blue")){
            for(int row = colRow;row<(colRow+tileDimension);row++){
                for(int column = colCol;column<(colCol+tileDimension);column++){
                    Color color = collagePicture.get(column, row);
                    int b = color.getBlue();
                    collagePicture.set(column,row,new Color(0,0,b));
                }
            }
        }
        if(component.equals("red")){
            for(int row = colRow;row<(colRow+tileDimension);row++){
                for(int column = colCol;column<(colCol+tileDimension);column++){
                    Color color = collagePicture.get(column, row);
                    int r = color.getRed();
                    collagePicture.set(column,row,new Color(r,0,0));
                }
            }
        }
        if(component.equals("green")){
            for(int row = colRow;row<(colRow+tileDimension);row++){
                for(int column = colCol;column<(colCol+tileDimension);column++){
                    ...
                    collagePicture.set(column,row,new Color(0,g,0));
                }
            }
        }
        // WRITE YOUR CODE HERE
    ...
    public void replaceTile (String filename,  int collageCol, int collageRow) {
        ...
        scale(replace,newTile);
        int colCol = collageCol*tileDimension;
        int colRow = collageRow*tileDimension;
        for(int row = colRow;row<(colRow+tileDimension);row++){
            for(int column = colCol;column<(colCol+tileDimension);column++){
                collagePicture.set(column,row,newTile.get(column%tileDimension,row%tileDimension));
            }
        }
        // WRITE YOUR CODE HERE
    }
    ...
    public void grayscaleTile (int collageCol, int collageRow) {
        ...
        for(int row = colRow;row<(colRow+tileDimension);row++){
            for(int column = colCol;(column<colCol+tileDimension);column++){
                Color g = toGray(collagePicture.get(column,row));
                collagePicture.set(column,row,g);
            }
        }
